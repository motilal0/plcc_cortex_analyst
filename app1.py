...
def send_message(prompt: str) -> Dict[str, Any]:
    """Calls the REST API and returns the response, preserving all prior conversation context."""
    # Construct the full conversation history
    messages = [{"role": "user", "content": [{"type": "text", "text": prompt}]}]
    messages.extend(st.session_state.messages)  # Include all previous messages
